...
namespace Contracts.Contracts.MyNFT
{
    public partial class MyNFTService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, MyNFTDeployment myNFTDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<MyNFTDeployment>().SendRequestAndWaitForReceiptAsync(myNFTDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, MyNFTDeployment myNFTDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<MyNFTDeployment>().SendRequestAsync(myNFTDeployment);
        }

        public static async Task<MyNFTService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, MyNFTDeployment myNFTDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, myNFTDeployment, cancellationTokenSource);
            return new MyNFTService(web3, receipt.ContractAddress);
        }
        ...
        public MyNFTService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
            ContractHandler = web3.Eth.GetContractHandler(contractAddress);
        }

        public MyNFTService(Nethereum.Web3.IWeb3 web3, string contractAddress)
        {
            Web3 = web3;
            ContractHandler = web3.Eth.GetContractHandler(contractAddress);
        }
        ...
        public Task<string> SafeMintRequestAsync(string to, string uri, BigInteger tokenPrice)
        {
            var safeMintFunction = new SafeMintFunction();
                safeMintFunction.To = to;
                safeMintFunction.Uri = uri;
                safeMintFunction.TokenPrice = tokenPrice;
            
             return ContractHandler.SendRequestAsync(safeMintFunction);
        }

        public Task<TransactionReceipt> SafeMintRequestAndWaitForReceiptAsync(string to, string uri, BigInteger tokenPrice, CancellationTokenSource cancellationToken = null)
        {
            var safeMintFunction = new SafeMintFunction();
                safeMintFunction.To = to;
                safeMintFunction.Uri = uri;
